...
public class Main {
    ...
    // This is the grid array that will be used in the game to store all of the
    // grids info.
    // 0 = Not Taken, 1 = White Occupation, 2 = Black Occupation.
    private static int[][] Grid = new int[8][8];

    // This stores the last move made so that the programme can change the grid
    ...
    public static void main(String[] args) {
        ...
        // This is where the method that will run the secret bot training program will
        // hide.
    ...
    private static void AIVSUserGame(String SPlayer1, String SPlayer2) {
        ...
        while (BStartChosen == false) {
            String UserInput = ReadUser(
                    "Would you like to start(enter: p) or would you like the bot to start(enter: b): ");
    ...
    public static int PostMoveModification(int[] LocalLastMove, int PlayerToken, boolean BDummy) {
        ...
        for (int i = 0; i <= 7; i++) {
            ...
            // This if Statement works out the directional x and y modifiers based on the
            // turn.
            int[] xyaddition = DirectionalModifiers(i);
            int xadition = xyaddition[0];
            int yadition = xyaddition[1];

            // Finds the cords inbetween the last cords and the next user token in the
            // direction iterating.
